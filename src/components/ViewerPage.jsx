...
export default function ViewerPage({ boardId }) {
  ...
  function handleExport(type) {
    try {
      let result
      if (type === 'png') {
        result = exportAsPNG(elements, board.boardWidth || 1200, board.boardHeight || 1600)
      } else {
        result = exportAsPDF(elements, board.boardWidth || 1200, board.boardHeight || 1600)
      }
    ...
    return (
      <div className="error-page">
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
          <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="var(--a)" strokeWidth="2" style={{ animation: 'spin 1s linear infinite' }}>
  ...
  const bw = board.boardWidth || 1200
  const bh = board.boardHeight || 1600
            ...
            <div className="dropdown-menu" style={{ minWidth: 220 }}>
              {exportReady ? (
                <div style={{ padding: 16, textAlign: 'center' }}>
                  <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#22c55e" strokeWidth="2.5" style={{ margin: '0 auto 8px', display: 'block' }}>
                  ...
                  <div style={{ marginTop: 10 }}>
                    <button className="btn-ghost" onClick={() => setExportReady(null)} style={{ fontSize: 11, padding: '3px 8px' }}>
                      ← Back
                    </button>
                  </div>
                </div>
